...
void Scheduler::add_new_process(const string& name, int memory_size, optional<vector<Instruction>> instructions_opt) {
    ...
    if (instructions_opt) {
        ...
        potential_total = final_instructions.size(); // Simplified total for custom instructions
    } else {
        random_device rd;
        mt19937 gen(rd());
        ...
        final_instructions = generate_instructions(num_instructions, declared_vars, 0, potential_total);
    }
    ...
    auto new_proc = make_shared<Process>(next_pid++, name, move(final_instructions), static_cast<size_t>(potential_total), string(buffer));
    
    // Set memory size on the process object and create its virtual address space
    // new_proc->memory_size = memory_size;
    // memory_manager->create_virtual_memory(new_proc, memory_size); // We will implement this next
...
vector<Instruction> Scheduler::generate_instructions(
    ...
    vector<string>& declared_vars,
    int depth,
    int& potential_total_instructions
) {
    random_device rd;
    mt19937 gen(rd());
...
void Scheduler::main_scheduler_loop() {
    while (!is_shutting_down) {
        if (is_scheduler_running.load()) {
            ...
            // Periodically check for processes that were waiting on a page fault
            // and move them back to the ready queue. This simulates I/O completion.
            {
    ...
    while (!is_shutting_down) {
       shared_ptr<Process> current_process;

        {
            ...
            if (!is_scheduler_running.load()) continue;
        ...
        while (!current_process->is_finished.load() && !is_shutting_down) {
            ...
            // uncomment this line in the final step
...
uint64_t Scheduler::get_total_ticks() const {
    // Total ticks is the system-wide tick count multiplied by the number of CPUs
